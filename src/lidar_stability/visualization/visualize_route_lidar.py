#!/usr/bin/env python3
"""
VisualizaciÃ³n 2D interactiva de ruta map-matched sobre nube LiDAR (LAZ).
"""
...
def visualize_route_on_lidar(mapmatch_path: str, laz_dir: str = None, tif_dir: str = None, source: str = "auto",
    ...
    fig.add_trace(go.Scatter(
        ...
        marker=dict(
            ...
            cmax=z_max_val,
            showscale=True,
            colorbar=dict(title="ElevaciÃ³n (m)")
        ),
        name='LiDAR',
        hoverinfo='skip',
        showlegend=False
    ...
    if available_features and show_terrain_features:
        title_str += f"<br><sub>CaracterÃ­sticas: {', '.join(available_features[:5])}</sub>"
