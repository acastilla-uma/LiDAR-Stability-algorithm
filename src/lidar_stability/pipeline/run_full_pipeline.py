...
"""
Pipeline completo:
raw -> processed-data -> map-matched -> featured -> visualizaciÃ³n 2D + 3D.
"""
...
def main() -> int:
    ...
    try:
        ...
        if not args.skip_mapmatching:
            print(f"\nðŸ”„ Sequential processing for {len(processed_files)} file(s): map-matched -> featured")
            for processed_file in processed_files:
                print(f"\n  â–¶ {processed_file.name}")
                run_cmd([
                    sys.executable,
                ...
                if not args.skip_terrain_features:
                    ...
                    try:
                        ...
                        print(f"  âš ï¸ Warning: Terrain feature extraction failed for {mapmatched_file.name}: {e}")
                        # Continue with visualization even if feature extraction fails
        else:
            print("\n[SKIP] processed -> map-matched")
            if not args.skip_terrain_features:
                print("\nðŸ“Š Extracting terrain features from existing map-matched files...")
                ...
                for mmf in mapmatch_files:
                    ...
                    try:
                        ...
                        print(f"  âš ï¸ Warning: Terrain feature extraction failed for {mmf.name}: {e}")
                        # Continue with visualization even if feature extraction fails
        ...
        if args.build_enhanced_gt:
            print("\nðŸ§© Building sprint-5 enhanced ground truth...")
            run_cmd([
                sys.executable,
                "src/lidar_stability/pipeline/build_enhanced_ground_truth.py",
        ...
        if args.train_w_model:
            print("\nðŸ¤– Training sprint-5 baseline model for w prediction...")
            input_glob = args.w_model_input_glob or f"{featured_dir.as_posix()}/{args.base}*.csv"
            run_cmd([
                sys.executable,
                "src/lidar_stability/ml/train_w_model.py",
        ...
        run_cmd([
            sys.executable,
            "src/lidar_stability/visualization/visualize_route_lidar.py",
        ...
        run_cmd([
            sys.executable,
            "src/lidar_stability/visualization/visualize_3d_interactive.py",
            "--base", args.base,
