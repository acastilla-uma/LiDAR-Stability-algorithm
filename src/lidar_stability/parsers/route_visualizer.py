...
def load_route_data(csv_path):
    ...
    if si_col is None:
        raise ValueError(f"No se encontrÃ³ columna de SI. Columnas disponibles: {df.columns.tolist()}")
    ...
    if df.empty:
        raise ValueError("No hay datos vÃ¡lidos despuÃ©s de limpiar")
    
    print(f"  âœ“ {len(df):,} puntos vÃ¡lidos")
    print(f"  âœ“ SI rango: {df['si'].min():.3f} - {df['si'].max():.3f}")
...
def build_map(segments_data, output_path):
    ...
    segments_info = "<br>".join([f"â€¢ {name}: {len(df):,} pts" for name, df in segments_data[:5]])
    if len(segments_data) > 5:
        segments_info += f"<br>â€¢ ... y {len(segments_data) - 5} mÃ¡s"
    ...
    <div style="position: fixed;
        ...
        <p style="margin: 0 0 8px 0; font-weight: bold;">Ãndice de Estabilidad (SI)</p>
...
def main():
    ...
    parser.add_argument(
        "--no-browser",
        action="store_true",
        help="No abrir automÃ¡ticamente el navegador",
    )
    ...
    print(f"\n{'='*70}")
    print("VISUALIZACIÃ“N DE RUTA(S) DOBACK CON ÃNDICE DE ESTABILIDAD")
    print(f"{'='*70}\n")
    ...
    for pattern in args.csv_files:
        ...
        if matches:
            ...
            if len(matches) > 1:
                print(f"ðŸ“ PatrÃ³n '{Path(pattern).name}' expandido a {len(matches)} archivos")
        else:
            print(f"âš  No se encontraron archivos para: {pattern}")
    
    if not all_files:
        print("âŒ No se encontraron archivos CSV vÃ¡lidos")
        return
    ...
    for csv_path in all_files:
        try:
            ...
            print(f"âš  Error cargando {csv_path.name}: {e}")
            continue
    
    if not segments_data:
        print("âŒ No se pudo cargar ningÃºn archivo vÃ¡lido")
        return
    
    build_map(segments_data, output_path)
    
    print(f"\nâœ“ Mapa guardado en: {output_path}")
    print(f"{'='*70}\n")
