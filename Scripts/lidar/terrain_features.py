...
class TerrainFeatureExtractor:
    ...
    def compute_slope_aspect(dem: np.ndarray, resolution: float = 1.0) -> Tuple[np.ndarray, np.ndarray]:
        ...
        dz_dx = convolve(dem_filled, sx, mode='same')
        dz_dy = convolve(dem_filled, sy, mode='same')
    ...
    def compute_phi_lidar(dem: np.ndarray, vehicle_track: float = 2.48,
        ...
        Returns:
            φ_lidar in radians
        """
        slope, aspect = TerrainFeatureExtractor.compute_slope_aspect(dem, resolution)
        
        # Assume vehicle moves in direction of max slope
        # Then transverse slope is perpendicular (90° rotated aspect)
        # For simplicity, use mean absolute slope as proxy
        phi_lidar = np.mean(np.abs(slope))
