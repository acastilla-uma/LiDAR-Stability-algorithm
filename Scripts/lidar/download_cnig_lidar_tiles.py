...
def main(argv: Optional[Iterable[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Descarga teselas LAZ de CNIG (LiDAR 3ª cobertura) a partir de una lista de nombres."
    )
    parser.add_argument(
        "--list",
        type=Path,
        default=Path("output/cnig_missing_tiles_all_doback.txt"),
        help="Fichero con nombres de teselas (.laz), una por línea.",
    )
    parser.add_argument(
        "--dest",
        type=Path,
    ...
    parser.add_argument("--timeout", type=float, default=120.0, help="Timeout por petición (segundos).")
    parser.add_argument("--retries", type=int, default=3, help="Reintentos por petición.")
