...
Usage:
    python Scripts/physics/compare_stability_csv.py --csv path/to/file.csv
"""
...
def _omega_from_yaw_deg(yaw_deg: np.ndarray, time_seconds: np.ndarray) -> np.ndarray:
    """Estimate angular rate (deg/s) from yaw angle (deg) using dψ/dt."""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Calcula SI con ecuación física y lo compara con SI del CSV.",
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Modo de cálculo: firmware (.ino) o roll/omega",
    )
    ...
    parser.add_argument(
        "--omega-from-yaw",
        action="store_true",
        help="Interpretar la columna omega como yaw (ángulo) y derivar d(yaw)/dt en deg/s",
    )
    parser.add_argument(
        "--yaw-scale",
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=120.0,
        help="Clipping absoluto para omega en deg/s tras conversión",
    )
    parser.add_argument(
        "--fit-k",
        action="store_true",
        help="Ajustar k1 y k2 por mínimos cuadrados antes de comparar",
    )
    parser.add_argument(
        "--fit-bias",
        action="store_true",
    ...
    parser.add_argument(
        "--forensic",
        action="store_true",
        help="Exportar diagnóstico detallado (términos intermedios y análisis de lag)",
    )
    parser.add_argument(
        "--forensic-csv",
        default=None,
    ...
    parser.add_argument(
        ...
        type=int,
        default=25,
        help="Lag máximo (muestras) para análisis temporal forense",
    )
    ...
    if work_df.empty:
        raise ValueError("No hay filas válidas tras eliminar NaN en roll/SI")

    omega_deg_s = None
    omega_source_desc = "no encontrada (ω=0)"
    if (not use_ino_mode) and omega_col:
        ...
        if args.max_omega_deg_s is not None and args.max_omega_deg_s > 0:
            ...
            omega_source_desc += f", clip=±{args.max_omega_deg_s} deg/s"
    ...
    if use_ino_mode:
        ...
        if gy_col:
            ...
            omega_source_desc = "sin gy (término dinámico=0)"
    ...
    try:
        ...
        plt.plot(x, work_df["si_calculada"], label="SI calculada (física)", linewidth=1.2)
        plt.xlabel(x_label)
        plt.ylabel("Stability Index (SI)")
        plt.title("Comparación de estabilidad: SI calculada vs SI en CSV")
        ...
        try:
            ...
            fig.add_trace(go.Scatter(x=x, y=work_df["si_calculada"], mode="lines", name="SI calculada (física)"))
            fig.update_layout(
                title="Comparación de estabilidad: SI calculada vs SI en CSV",
                xaxis_title=x_label,
            ...
            raise ModuleNotFoundError(
                "No hay librería de gráficos disponible. Instala matplotlib o plotly."
            ) from exc
    ...
    print("\n=== Resultado comparación SI ===")
    print(f"Archivo entrada: {csv_path}")
    print(f"Modo cálculo: {'ino' if use_ino_mode else 'roll'}")
    if use_ino_mode:
    ...
    print(f"Parámetros usados: c0={c0_used:.6f}, k1={k1_used:.6f}, k2={k2_used:.6f}, omega_crit={engine.omega_crit_rad_s:.6f} rad/s")
    print(f"Ajuste automático: {'sí' if args.fit_k else 'no'}")
