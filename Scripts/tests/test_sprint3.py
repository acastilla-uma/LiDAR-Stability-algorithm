...
from lidar.tif_reader import TIFReader
from lidar.terrain_provider import TerrainProvider
...
class TestLAZReader:
    ...
    def test_laz_reader_init(self, sample_laz_path):
        ...
        logger.info(f"✓ LAZ reader loaded {len(reader.points)} points")
    ...
    def test_laz_reader_bounds(self, sample_laz_path):
        ...
        logger.info(f"✓ LAZ bounds: X=[{xmin:.0f}, {xmax:.0f}], Y=[{ymin:.0f}, {ymax:.0f}]")
    ...
    def test_laz_reader_stats(self, sample_laz_path):
        ...
        logger.info(f"✓ LAZ stats: {stats['n_points']} points, Z: [{stats['z_range'][0]:.1f}, {stats['z_range'][1]:.1f}]")
    ...
    def test_laz_extract_patch(self, sample_laz_path):
        ...
        logger.info(f"✓ Patch extraction: {len(patch)} points in radius")
    ...
    def test_laz_extract_patch_knn(self, sample_laz_path):
        ...
        logger.info(f"✓ Extracted {k} nearest neighbors")
...
class TestTIFReader:
    ...
    def test_tif_reader_init(self, sample_tif_path):
        ...
        logger.info(f"✓ TIF reader loaded {reader.data.shape[0]}x{reader.data.shape[1]} pixels")
    ...
    def test_tif_reader_bounds(self, sample_tif_path):
        ...
        assert xmin < xmax
        assert ymin < ymax
        logger.info(f"✓ TIF bounds: X=[{xmin:.0f}, {xmax:.0f}], Y=[{ymin:.0f}, {ymax:.0f}]")
    ...
    def test_tif_reader_stats(self, sample_tif_path):
        ...
        logger.info(f"✓ TIF stats: Z=[{stats['z_min']:.1f}, {stats['z_max']:.1f}]")
    ...
    def test_tif_get_elevation(self, sample_tif_path):
        ...
        logger.info(f"✓ Elevation at ({x:.0f}, {y:.0f}): {z}")
    ...
    def test_tif_extract_patch(self, sample_tif_path):
        ...
        if patch is not None:
            assert isinstance(patch, np.ndarray)
            assert patch.ndim == 2
            logger.info(f"✓ Extracted TIF patch: {patch.shape}")
...
class TestTerrainProvider:
    ...
    def test_provider_init(self, terrain_provider):
        ...
        logger.info(f"✓ Provider initialized with {min(3, len(provider.laz_readers))} LAZ files (showing limit)")
    ...
    def test_provider_coverage(self, terrain_provider):
        ...
        logger.info(f"✓ Coverage info: {info['n_laz_files']} LAZ files, " +
    ...
    def test_provider_point_coverage(self, terrain_provider):
        ...
        logger.info(f"✓ Coverage check works")
...
class TestTerrainFeatures:
    ...
    def test_compute_phi_lidar(self, sample_dem):
        ...
        logger.info(f"✓ φ_lidar = {phi:.4f} rad = {np.degrees(phi):.2f}°")
    ...
    def test_extract_features_nodata(self):
        ...
        logger.info(f"✓ All-NaN DEM handled correctly")
    ...
    def test_extract_features_from_point_cloud(self):
        ...
        logger.info(f"✓ Features from point cloud: φ={features['phi_lidar']:.4f}, TRI={features['tri']:.2f}")
